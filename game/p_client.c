...
// USE - special actions taken when +use is pressed
	if(!client->use && (ucmd->buttons & BUTTON_USE))
	{
		...
		else
		{
			...
			{
				//BC change interact range since it was kinda low. was 35. 10-29-2018

				
				

				if(!stricmp(viewing->classname,"monster_olive") && range <= INTERACT_RANGE)
					olive_action(viewing,ent);

				if(!stricmp(viewing->classname,"monster_worker") && range <= INTERACT_RANGE)
					worker_action(viewing,ent);


				if(!stricmp(viewing->classname,"item_doorknob") && range <= INTERACT_RANGE)
					doorknob_action(viewing,ent);

				if(!stricmp(viewing->classname,"item_lever") && range <= INTERACT_RANGE)
					lever_action(viewing,ent);

				if(!stricmp(viewing->classname,"item_wallbutton") && range <= INTERACT_RANGE)
					wallbutton_action(viewing,ent);


				if(!stricmp(viewing->classname,"item_suitcase") && range <= INTERACT_RANGE)
					suitcase_action(viewing,ent);


				if(!stricmp(viewing->classname,"item_pickable") && range <= INTERACT_RANGE)
					pickable_action(viewing,ent);

				if(!stricmp(viewing->classname,"func_usebutton") && range <= INTERACT_RANGE)
